...
import torch
import numpy as np
# NOTE (PACE local vendoring): json5 / pyflexfloat / humanize imports dropped.
# They are only used by data-generation helpers that this PACE flow does not call
# (config loading, FP8 flexfloat emission, heap-size reporting). The formatting
# functions used by datagen.py (ctype/numpy_type/hex_ctype/_integer_precision_t,
# emit_license, format_array_declaration/definition) need only numpy + stdlib.
...
def type_to_precision_t(dtype):
    ...
    # torch.float8_e4m3fn only exists on torch>=2.1; skip on older versions (FP8 unused here).
    if hasattr(torch, "float8_e4m3fn"):
        torch_type_to_precision_t_map[torch.float8_e4m3fn] = 1
...
def float_to_hex(array, prec):
    ...
    if prec == np.uint32:
        i32 = np.asarray(array, np.uint32)
        bits = i32.view(np.uint32).item()
        return f"0x{bits:08X}"
    if prec == np.uint16:
        i16 = np.asarray(array, np.uint16)
        bits = i16.view(np.uint16).item()
        return f"0x{bits:04X}"
...
def format_array_initializer(dtype, array, hex_format=False):
    ...
    ctype_to_prec = {
        ...
        '__fp16':  np.float16,
        '__fp8':   "bfloat16",   
        # treat fp8 as BF16-style hex for consistency
    }
    ...
    if prec is None:
        prec = ctype_to_np.get(dtype, None)
    if prec is None:
