/*
 * 8-core version of exp_hwfpu_fp16: same 1024 inputs, split 128/core, each core's
 * expf() on its own FPU. One barrier: before core 0 checks the full result array.
 */
...
int main(void) {
  ...
  printf("HW-FPU expf core %u compute cycles = %u (avg %u cyc/elem over %d elems)\n",
         core, compute_end - compute_start, (compute_end - compute_start) / CHUNK, CHUNK);

  synch_barrier();  // every core's slice of ofmap_hwfpu[] must be written before core 0 checks it

  if (core != 0) return 0;

  int errors = check_output(ofmap_hwfpu, golden, INPUTS_LEN);
  printf("HW-FPU expf 8-core errors = %d\n", errors);
  return errors;
}
